...
def new_word():
    global word
    word = random.choice(words_dict)
    word_label.config(text=f"{word['Japanese']}")
    countdown(3)
...
#NEEDS THIS FUNCTION TO MAKE THE LABELS BACK IN JAPAHESE
def set_word():
    lang_label.config(text='Japanese', bg='white')
    word_label.config(text='', bg='white')
...
wrong_button = Button(image=x_mark, width=100, height=100,
                      highlightthickness=0, borderwidth=0, bg=BACKGROUND_COLOR)
